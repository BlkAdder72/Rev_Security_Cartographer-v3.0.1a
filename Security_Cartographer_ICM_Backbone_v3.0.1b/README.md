...
**New here? Start with [`START_HERE.md`](START_HERE.md).** It explains the demo, safe demo deletion,
and real ICM installation in short, plain-language steps.
...
The three deployable ICM layers remain separate from the optional, self-contained demonstration:
...
demo/                       Optional local simulation; delete after evaluation
├── README.md
├── DEMO_GUIDE.md
├── SUBMISSION.md
├── RUN_DEMO.bat
...
Only `instructions/`, `context/`, and `memory/` are merged into an existing ICM agent. The `demo/`
folder is not part of the deployed agent and may be deleted at any time.

## Optional demo mode

The demo imports `security_cartographer.py` directly from
`context/security-cartographer/runtime/`. It does not ship or execute a second copy of the engine.

1. Extract the complete package without changing its directory relationships.
2. Open `demo/README.md` or `demo/DEMO_GUIDE.md`.
3. On Windows, run `demo/RUN_DEMO.bat`.
4. On macOS or Linux, run `sh demo/run_demo.sh`.
5. Inspect the maps and verified ICM bundle under `demo/demo_output/`.
6. Delete the complete `demo/` folder when evaluation is finished.

Deleting `demo/` does not remove or alter the deployable ICM module.
...
The project is distributed under the MIT License included in the root `LICENSE` file.
